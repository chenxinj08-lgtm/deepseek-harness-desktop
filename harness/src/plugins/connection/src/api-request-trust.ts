...
interface ApiTrustRequest {
  ...
  /** TCP 对等地址(node:http req.socket.remoteAddress)。缺省视为未知(不收紧)。 */
  remoteAddress?: string
}

/**
 * Whether a socket peer address is loopback. Covers IPv4 127/8、IPv6 ::1,
 * 与 IPv4-mapped IPv6(::ffff:127.x.x.x)。
 */
 ...
 * @param request - Node HTTP or Fetch request facts (headers, optional socket peer address).
...
export function isTrustedApiRequest(request: ApiTrustRequest, trustedHosts: readonly string[]): boolean {
  ...
  // Peer-address fence:LAN 客户端可伪造 Host 头绕回环判断。当对等地址已知且
  // 非回环时,Host 必须命中 trustedHosts —— 默认 127.0.0.1 绑定下对等地址恒回环,
  // 行为零变化;LAN 部署则要求请求真实来自声明的主机。
  if (request.remoteAddress !== undefined && !isLoopbackAddress(request.remoteAddress)
