    ...
    sessions: {
      ...
      prompt: (request) => {
        ...
        const durable: ContentBlock[] = content.map((block) => {
          ...
          if (block.type === 'local-file') {
            // Fixture has no host store to resolve metadata; pass the id through
            // with neutral fields so prompt plumbing tests can observe the block.
            return {
              type: 'local-file',
              id: block.id,
              name: 'fixture',
