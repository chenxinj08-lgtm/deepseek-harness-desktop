...
/** WS 发送高水位(1 MiB):超过即等 drain,慢消费者被 socket 背压,不再无界堆积。 */
...
/** 发送一帧;socket 缓冲超过高水位时等 drain 再继续(原生背压,内存有界)。 */
async function sendWithBackpressure(socket: WebSocket, frame: RpcRequest<Frame>): Promise<void> {
  if (socket.readyState === WebSocket.OPEN && socket.bufferedAmount > SEND_HIGH_WATER) {
...
export class WebSocketDownlinks {
  ...
  private async pump<F extends Frame>(
    ...
    try {
      for await (const frame of frames) await sendWithBackpressure(socket, frame)
    } catch (error) {
