...
/**
 * A durable reference to a locally imported file. The bytes live outside the
 * prompt; only this metadata rides the message. The wire serializer renders it
 * into a bounded `<local_file …/>` reference the model can resolve through the
 * local_file_* tools, never the raw file name (prompt-injection safe).
 */
export interface LocalFileBlock {
  type: 'local-file'
  /** Host-validated local-files UUID. */
  id: string
  /** Host-resolved display name. */
  name: string
  /** Encoded byte length. */
  sizeBytes: number
  /** Host-detected media type (never trusted from the extension alone). */
  mediaType: string
  /** Host-detected content kind. */
  kind: 'text' | 'csv' | 'xlsx' | 'docx' | 'binary'
}
