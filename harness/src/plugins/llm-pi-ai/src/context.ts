...
import { CallId, contentHasImage, flattenModelText, LlmError } from '@deepseek-ai/dsh-llm'
import type { ContentBlock, GenerateOptions, Message } from '@deepseek-ai/dsh-llm'
import type { AttachmentStore } from '@deepseek-ai/dsh-attachment'
import type { Context as PiContext, ImageContent, Message as PiMessage, TextContent, Tool as PiTool } from '@earendil-works/pi-ai'
import { toPiAssistant } from './replay.ts'

/** Join the text blocks of a harness message, serializing local-file references. */
function flattenText(message: Message): string {
  return flattenModelText(message.content)
}
...
async function userContent(
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'local-file':
        // pi-ai has no native local-file vocabulary; fold the bounded
        // reference into the text stream so the model can resolve it through
        // the local_file_* tools.
        content.push({ type: 'text', text: flattenModelText([block]) })
        break
      case 'tool-result':
        {
