...
export interface IConversation {
  ...
  /**
   * Validate and stage browser image files in the caller scope's composer.
   * @param files - browser files using a supported image MIME type.
   * @returns whether the input machine accepted the complete batch.
   */
  stageDraftImages(files: readonly File[]): boolean
  /**
   * Sum live browser-owned draft image bytes for an ordered id set.
   * @param ids - draft image ids from the scoped input snapshot.
   * @returns bytes owned by descriptors that still exist.
   */
  draftImageBytes(ids: readonly DraftAttachmentId[]): number
  /**
...
export class ConversationController extends Service implements IConversation {
  ...
  async sendSession(
    ...
    // Structured pre-send injection: attachments submit as prompt parts
    // alongside (never spliced into) the text, so the composer draft and the
    // logged message body stay free of internal serialization markup. serial()
    // short-circuits on the first non-null return; without a listener it
    // resolves undefined and parts stay empty. A throwing listener blocks the
    // send (hub restores the draft).
    const injectedParts = await (this.ctx as unknown as {
      serial: (name: string, ...args: unknown[]) => Promise<unknown>
    }).serial('conversation/send-content-inject', session.sessionId, mode)
    const parts = Array.isArray(injectedParts) ? injectedParts : []
    const content = [...uploaded, ...parts, ...(text === '' ? [] : [{ type: 'text' as const, text }])]
  ...
  /** Validate, preview, and append browser image files to the scoped composer. */
  stageDraftImages(files: readonly File[]): boolean {
    const id = this.scopeId('stageDraftImages')
    const actx = this.requireSessions().scope(id)
    if (actx === undefined) throw new Error(`conversation.stageDraftImages: session "${id}" resolved no scope`)
    const images = this.createDraftImages(files)
    if (this.input.for(actx).addImages(images.map(image => image.id))) return true
  ...
  /** Sum the byte sizes of live browser-owned draft images. */
  draftImageBytes(ids: readonly DraftAttachmentId[]): number {
    return this.draftImages(ids).reduce((total, attachment) => total + attachment.file.size, 0)
  }
