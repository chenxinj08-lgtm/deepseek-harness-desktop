...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * Action strip attached to one finalized user message, rendered beside
     * that message's action row. The chat entry owns the render site and
     * passes the message's seq and plain text; contributors add per-message
     * actions (edit and similar) without importing the conversation
     * implementation. Entries render by ascending `order`.
     */
...
/**
 * Owner currency of a user-message action strip (edit and similar), addressed
 * by the message's event seq and its plain text. Contributors add per-message
 * actions without importing the conversation implementation.
 */
export interface UserActionOwnerProps {
  /** Fork anchor: the turn-end seq before this message (the edit forks at it). */
  seq: number
  /** The message's own seq: inline-editor identity. */
  messageSeq: number
  /** Plain text of the user message (text blocks joined); empty for attachment-only rows. */
  text: string
  /** The message's content blocks (attachments re-send with the edited text). */
  content: readonly unknown[]
  /**
   * True when the message has no previous completed turn to fork at (the
   * session's first message). The save flow then creates a blank session
   * instead of forking, so the edited text truly replaces the original.
   */
  isFirst: boolean
}
