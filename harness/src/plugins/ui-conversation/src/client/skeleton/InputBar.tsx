...
import { stagedFor, subscribeStaged } from '../staged.ts'
import {
...
export function InputBar({
  ...
  const empty = draft.trim() === '' && attachments.length === 0 && !staged
  const canSend = !empty
  ...
  const onPrimary = (): void => {
    ...
    if (canSend && !disabled && !machineBusy) inputActions.submit()
  }
          ...
          <div className={css.trailing}>
            ...
            <Tooltip label={primaryLabel} side="top" delayMs={500}>
              <button
                ...
                disabled={primaryStops ? stop === undefined : !canSend || disabled || machineBusy}
                onMouseDown={keepFocus}
                onClick={onPrimary}
              >
                {primaryStops ? (
