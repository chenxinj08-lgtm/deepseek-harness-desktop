...
export class InputMachine {
  ...
  /** Staged local files present: empty drafts may still send when true. */
  ...
  /** Track staged local-file presence so empty drafts may still send. */
  setStaged(present: boolean): void {
    this.staged = present
  }
  ...
  private onEnter(mode: InputSubmitMode): InputEffect[] {
    ...
    // Empty drafts send only when local files are staged (the composer enables
    // sending with attachments but no text); otherwise the machine refuses.
    if (trimmed === '' && !this.staged) return []
    this.paste = undefined
