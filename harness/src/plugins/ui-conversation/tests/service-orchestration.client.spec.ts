...
describe('ConversationController', () => {
  ...
  it('stages image previews through the public scoped service face', async () => {
    const b = await bench()
    const created = vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:public-stage')
    try {
      expect(b.scoped.stageDraftImages([
        new File([Uint8Array.of(1)], 'pasted.png', { type: 'image/png' }),
      ])).toBe(true)
      expect(b.shell.snapshot.imageIds).toHaveLength(1)
      expect(b.scoped.draftImageBytes(b.shell.snapshot.imageIds)).toBe(1)
      expect(() => b.root.stageDraftImages([])).toThrow(/requires a session scope/u)
    } finally {
      created.mockRestore()
    }
    await b.runtime.dispose()
