...
/** Minimal local-file service face the prompt admission needs (optional composition). */
interface LocalFileStore {
  get(cwd: string, id: unknown): Promise<{ metadata: { id: string; name: string; size: number; mediaType: string; kind: 'text' | 'csv' | 'xlsx' | 'docx' | 'binary' } }>
  parseId(value: string): unknown
}

/** Validate one prompt as a batch before publishing any durable image object. */
async function durablePromptContent(ctx: Context, content: readonly PromptContentPart[], cwd: string | undefined): Promise<ContentBlock[]> {
  ...
  const localFileParts = content.filter((part): part is Extract<PromptContentPart, { type: 'local-file' }> => part.type === 'local-file')
  const localFiles = ctx.get('localFiles') as LocalFileStore | undefined
  if (localFileParts.length > 0 && (localFiles === undefined || cwd === undefined)) {
    throw new AttachmentError('Local-file references require the local-files service and a session workspace.', 'LOCAL_FILE_UNAVAILABLE')
  }
  const decoded = imageParts.map(part => ({ part, data: decodeBase64(part.data) }))
  ...
  for (const part of content) {
    ...
    if (part.type === 'local-file') {
      // Resolve the host-trusted record; a missing or cross-workspace id fails
      // closed rather than fabricating a file.
      try {
        const record = await localFiles!.get(cwd!, localFiles!.parseId(part.id))
        blocks.push({
          type: 'local-file',
          id: record.metadata.id,
          name: record.metadata.name,
          sizeBytes: record.metadata.size,
          mediaType: record.metadata.mediaType,
          kind: record.metadata.kind,
        })
      } catch {
        throw new AttachmentError(`Local file "${part.id}" was not found in this workspace.`, 'LOCAL_FILE_NOT_FOUND')
      }
      continue
    }
    // image: persist validated bytes to a durable reference.
...
/**
 * 有界异步队列:环缓冲(head 索引,出队 O(1)) + 高水位丢最旧 + 快照帧同键合并。
 * 快照帧(session/queue、session/projection、session/jobs)是完整状态,慢消费者
 * 只需最新值 —— 同会话同类型的新帧直接替换队尾,队列不再无限增长。
 */
class FrameQueue<F> {
  private buffer: F[] = []
  private head = 0
  ...
  constructor(
    private readonly snapshotKey: (item: F) => string = () => '',
    private readonly maxBuffered = 4096,
  ) {}

  push(item: F): void {
    if (this.done) return
    const key = this.snapshotKey(item)
    if (key !== '') {
      const tailIndex = this.buffer.length - 1
      if (tailIndex >= this.head && this.snapshotKey(this.buffer[tailIndex] as F) === key) {
        this.buffer[tailIndex] = item // 同键快照帧替换队尾
        this.waiter?.()
        return
      }
    }
    if (this.buffer.length - this.head >= this.maxBuffered) {
      // 高水位:物理丢弃最旧,缓冲保持有界(触发时 O(n),均摊 O(1))
      if (this.head > 0) { this.buffer = this.buffer.slice(this.head); this.head = 0 }
    ...
    try {
      while (true) {
        while (this.head < this.buffer.length) {
          ...
          this.head++
          // 定期 compact:丢弃已消费前缀,避免长会话中数组持续增长
...
/** 快照帧合并键:仅 session/projection(客户端按更高 seq 覆盖,设计上允许丢中间值)。
 * 其余帧(session/queue、session/jobs 等)是客户端状态机推进序列,逐帧必达,不合并。 */
function snapshotKey(frame: RpcRequest<MuxFrame>): string {
  const p = frame.payload
  return p.type === 'session/projection' ? `${p.type}:${p.sessionId}:${p.key}` : ''
}

/** Host 流无 higher-seq-wins 语义,全量快照帧逐帧必达,不合并。 */
function snapshotKeyHost(_frame: RpcRequest<HostFrame>): string {
  return ''
}
...
function historyPage(
  ...
  // 一次建立 callId → 工具信息索引,每事件配对 O(1)(原 backscanArgs 每事件 O(page))。
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        try {
          ...
          // 错误只上 message 不上堆栈(String(error) 不含 stack):领域细节供本机 UI 排查,
          // 敏感的内部细节仍留在宿主侧(搜索 provider 的底层异常不走此路径)。
          return err(request, {
            code: 'internal',
    ...
    events: {
      mux(_request, signal) {
        const queue = new FrameQueue<RpcRequest<MuxFrame>>(snapshotKey)
        muxQueues.add(queue)
      ...
      host(_request, signal) {
        const queue = new FrameQueue<RpcRequest<HostFrame>>(snapshotKeyHost)
