...
const sanitizeVersion = (v) => (typeof v === 'string' && SEMVER_RE.test(v)) ? v : null
const latestDsh = async () => {
  const d = await getJSON(REG)
  return d && d.version ? sanitizeVersion(d.version) : null
}
// 当前内置 dsh 版本
const builtinDsh = () => {
  try {
...
function makeWin() {
  ...
  win = new BrowserWindow({ ...opts, webPreferences: {
  ...
  win.webContents.setWindowOpenHandler(({ url }) => {
    // 协议白名单:只允许 https/http/mailto,防 file: / javascript: 等危险协议
    if (url.startsWith('https://') || url.startsWith('http://') || url.startsWith('mailto:')) shell.openExternal(url)
    return { action: 'deny' }
  })
...
app.whenReady().then(() => {
    // CSP: 只允许 127.0.0.1:3080 本地服务作为资源源
    session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
      const csp = [
        "default-src 'self' http://127.0.0.1:3080",
        "script-src 'self' http://127.0.0.1:3080 'unsafe-inline'",
        "style-src 'self' http://127.0.0.1:3080 'unsafe-inline'",
        "img-src 'self' http://127.0.0.1:3080 blob: data:",
        "font-src 'self' http://127.0.0.1:3080 data:",
        "connect-src 'self' http://127.0.0.1:3080 ws://127.0.0.1:3080 https://registry.npmjs.org",
        "object-src 'none'",
        "base-uri 'none'",
        "form-action 'none'",
